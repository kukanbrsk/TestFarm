...
public class BlockWheat : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.collider.TryGetComponent(out PlayerController player) && _isFree && !_UIController.limitBlocks && _delayDone)
        {
            gameObject.layer = LayerMask.NameToLayer("Not interactable");
            _rigidbodyBlock.isKinematic = true;
            player.BackpackPlayer.AddBlockToBackpack(this);
            _isFree = false;
        }
    }
